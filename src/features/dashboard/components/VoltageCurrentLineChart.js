...
function LineChart() {
  const [solarVoltageData, setSolarVoltageData] = useState(Array(10).fill(0));
  const [solarCurrentData, setSolarCurrentData] = useState(Array(10).fill(0));
  ...
  useEffect(() => {
    ...
    onValue(SolarRef, (snapshot) => {
      ...
      if (data) {
        setSolarVoltageData((prevData) => {
          const newData = [...prevData.slice(1), data.Voltage];
          return newData;
        });
        setSolarCurrentData((prevData) => {
  ...
  return (
    <TitleCard title={"Solar Voltage and Current"}>
